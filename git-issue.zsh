...
function percol_select_from_git_issue_list_to_git_flow_feature_start() {
  if $(is_issued) ; then
    echo "Loading..."
    TICKET=$(git-issue --no-color list | percol --match-method migemo | sed -e 's/^#\([0-9]*\).*/\1/')
...
function percol_select_from_git_issue_to_open() {
  if $(is_issued) ; then
    ...
    _ret=$?
    if [ $_ret -ne 0 ]; then
      TICKET=$(git-issue --query='status_id=*&limit=50&sort=updated_on:desc' --no-color  --oneline | percol --match-method migemo | sed -e 's/^#\([0-9]*\).*/\1/')
      ...
      TICKET=$(git-issue list --sort=updated --no-color --oneline | percol --match-method migemo | sed -e 's/^#\([0-9]*\).*/\1/')
